...
func (repo *Repository) TagCreate(name string, ref string, createTagObject bool) error {
	...
	*/
	...
	if createTagObject {
		tag := &GitTag{}
		tag.Kvlm = &sharedtypes.Kvlm{}
		(*tag.Kvlm)["object"] = [][]byte{[]byte(sha)}
		(*tag.Kvlm)["type"] = [][]byte{[]byte("commit")}
		(*tag.Kvlm)["tag"] = [][]byte{[]byte(name)}

		(*tag.Kvlm)["tagger"] = [][]byte{[]byte("test test@example.com")}
		(*tag.Kvlm)[""] = [][]byte{[]byte("A tag generated by wyag, which won't let you customize the message!")}
